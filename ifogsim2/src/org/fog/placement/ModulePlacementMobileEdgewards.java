...
public class ModulePlacementMobileEdgewards extends ModulePlacement{
	...
	public ModulePlacementMobileEdgewards(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, 
		...
		for(FogDevice dev : getFogDevices()){
			...
			for(AppModule module : getApplication().getModules()){
				getCurrentModuleInstanceNum().get(dev.getId()).put(module.getName(), 0);
			}
		}
	...
	private List<String> shiftModuleNorth(String moduleName, double cpuLoad, Integer deviceId, List<String> operatorsToPlace) {
		...
		for(String module : modulesToShift){
			double moduleLoad = getCurrentModuleLoadMap().get(deviceId).getOrDefault(module, 0.0);
			loadMap.put(module, moduleLoad);
			moduleToNumInstances.put(module, getCurrentModuleInstanceNum().get(deviceId).getOrDefault(module, 0) + 1);
			totalCpuLoad += moduleLoad;
			...
			if(getCurrentCpuLoad().get(id) + totalCpuLoad > fogDevice.getHost().getTotalMips()){
				...
				for(String module : _modulesToShift){
					if(!modulesToShift.contains(module)){
						int onDevice = getCurrentModuleInstanceNum().get(id).getOrDefault(module, 0);
						int shifting = moduleToNumInstances.getOrDefault(module, 0);
						moduleToNumInstances.put(module, onDevice + shifting);
						double moduleLoad = getCurrentModuleLoadMap().get(id).getOrDefault(module, 0.0);
						loadMap.put(module, moduleLoad);
						cpuLoadShifted += moduleLoad;
						totalCpuLoad += moduleLoad;
